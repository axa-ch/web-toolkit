...
const metalsmithSitemap = function() {
  return (files, metalsmith, done) => {
    ...
    for (let file in files) {
      let directories = file.split(path.sep)
      let parentNode = root
    ...
    root.children.push(sassdocNode)
    for (const name in metadata.sassdoc.comments) {
      const comment = metadata.sassdoc.comments[name] 
      sassdocNode.children.push({
        name: name,
        render: true,
        filename: 'sassdoc/' + comment.urlFriendlyName,
        title: name,
        order: 0,
      })
    }

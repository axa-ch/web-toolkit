...
export default function(cb) {
  ...
  let config = readJSONFile(path.join(cwd, './docs/config.json'));
  let metalsmithSource = './docs/page';

  // basedir for include paths
  let jadeBaseDir = path.join(cwd, metalsmithSource);
  ...
  let metalsmith = new Metalsmith(cwd);
    ...
    branch(['**/*.jade'])
      ...
      .use(function(files, metalsmith, done) {
        _.forEach(files, file => {
            return file.link = `${file.link.slice(0, -5)}.html`;
          }
        );
        return done();
      })
  ...
  let build = () =>
      metalsmith.build(function(err) {
        if (!err) { return cb(); } else { return cb(err); }
      })
    ;
  ...
  if (username && password) {
    ...
    return loadChangelog(username, password, function(err, changelog) {
        if (!err) {
